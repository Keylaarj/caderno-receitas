...
function obterReceitas() {
  ...
  if (!salvas || JSON.parse(salvas).length === 0) {
    localStorage.setItem('receitasUsuario', JSON.stringify(receitasPadrao));
...
function salvarReceitas(lista) {
  localStorage.setItem('receitasUsuario', JSON.stringify(lista));
}